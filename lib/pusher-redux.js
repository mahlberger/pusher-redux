...
module.exports.subscribe = function (channelName, eventName, actionType, additionalParams, callbacks = {}) {
  addToQueue(function () {
    ...
    if (!eventSubs[actionType]) {
      eventSubs[actionType] = function (rawData) {
        let data;
        if (callbacks.modifyRawData) {
          data = callbacks.modifyRawData(rawData);
        }
        else {
          data = rawData;
        }
        var t = pusherAction({ actionType: actionType, channelName: channelName, eventName: eventName, data: data, additionalParams: additionalParams });
        config.store.dispatch(t);
      };
